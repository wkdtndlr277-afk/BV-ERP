...
async function uploadPlanFile() {
  ...
  try {
    ...
    for (let i = 4; i < data.length; i++) {
      ...
      // 발주 수량 합계 계산
      const qtyCoupang = parseFloat(row[colMap['coupang']]) || 0;
      const qtyOasis = parseFloat(row[colMap['oasis']]) || 0;
      const qtyUiwang = parseFloat(row[colMap['uiwang']]) || 0;
      const qtyStore = parseFloat(row[colMap['store']]) || 0;
      const qtyFranchise = parseFloat(row[colMap['franchise']]) || 0;
      const qtyKurlyFrozen = parseFloat(row[colMap['kurly_frozen']]) || 0;
      const qtyKurlyPyeongtaek = parseFloat(row[colMap['kurly_pyeongtaek']]) || 0;
      const qtyKurlyGimpo = parseFloat(row[colMap['kurly_gimpo']]) || 0;
      const qtyKurlyChangwon = parseFloat(row[colMap['kurly_changwon']]) || 0;
      const qtyBaemin = parseFloat(row[colMap['baemin']]) || 0;
      const qtyNaver = parseFloat(row[colMap['naver']]) || 0;
      const qtyExtra = parseFloat(row[colMap['extra']]) || 0;
      ...
      // 재고
      const excelStock = parseFloat(row[colMap['stock']]) || 0;
      
      // 순번
      const seq = row[colMap['seq']] || null;
      if (seq && !isNaN(parseFloat(seq))) {
        seqNo = parseFloat(seq);
      }
...
function showAddFrozenStockModal() {
  closeModal();
  ...
  showModal('냉동재고 추가', `
    <form id="frozen-stock-form" class="space-y-4">
      <div>
        <label class="block text-sm font-medium text-gray-700 mb-1">제품명 <span class="text-red-500">*</span></label>
        <input type="text" id="frozen-product-name" required class="w-full px-3 py-2 border rounded-lg" placeholder="제품명 입력">
      </div>
      <div class="grid grid-cols-2 gap-4">
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-1">수량 <span class="text-red-500">*</span></label>
