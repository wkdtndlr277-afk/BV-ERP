...
// 발주서 업로드 후 자동으로 생산 스케줄 생성
productionPlanRoutes.post('/from-order', async (c) => {
  await ensureTables(c.env.DB);
  
  try {
    const body = await c.req.json();
    const { plan_date, channel, file_name, items } = body;
    // items: [{ product_code, product_name, quantity }]
    
    if (!items || items.length === 0) {
      return c.json({ success: false, error: '생산 등록된 항목이 없습니다.' }, 400);
    }
    
    const planDate = plan_date || new Date().toISOString().split('T')[0];
    const channelName = channel ? channel.toUpperCase() : '발주서';
    
    // 같은 날짜에 이미 생성된 스케줄이 있는지 확인
    const existingPlan = await c.env.DB.prepare(`
      SELECT id, plan_name FROM production_plan 
      WHERE plan_date = ? AND plan_name LIKE ?
    `).bind(planDate, `%${channelName}%`).first();
    ...
    if (existingPlan) {
      ...
      for (const item of items) {
        ...
        const existingItem = await c.env.DB.prepare(`
          SELECT id, order_total FROM production_plan_items 
          WHERE plan_id = ? AND product_code = ?
        `).bind(planId, item.product_code).first();
        
        if (existingItem) {
          // 수량 업데이트 (기존 + 신규)
          await c.env.DB.prepare(`
            UPDATE production_plan_items 
            SET order_total = order_total + ?, updated_at = CURRENT_TIMESTAMP
            WHERE id = ?
          ...
          `).bind(item.product_code).first<{ current_stock: number }>();
          
          const frozen = await c.env.DB.prepare(`
            SELECT SUM(quantity) as total FROM frozen_stock WHERE product_code = ?
          `).bind(item.product_code).first<{ total: number }>();
          
          const currentStock = product?.current_stock || 0;
          const frozenStock = frozen?.total || 0;
          const requiredQty = item.quantity - currentStock - frozenStock;
          
          await c.env.DB.prepare(`
            INSERT INTO production_plan_items (
              plan_id, product_name, product_code, order_total,
              current_stock, frozen_stock, required_qty, status
            ) VALUES (?, ?, ?, ?, ?, ?, ?, '대기')
          `).bind(
            planId, item.product_name, item.product_code, item.quantity,
            currentStock, frozenStock, requiredQty > 0 ? requiredQty : 0
          ).run();
        }
      }
      ...
      await c.env.DB.prepare(`
        ...
        SET total_items = (SELECT COUNT(*) FROM production_plan_items WHERE plan_id = ?),
            total_quantity = (SELECT SUM(order_total) FROM production_plan_items WHERE plan_id = ?),
            updated_at = CURRENT_TIMESTAMP
        WHERE id = ?
      ...
      `).bind(
        planDate,
        `${planDate} ${channelName} 발주 스케줄`,
        file_name || `${channelName}_order.xlsx`,
        items.length,
        totalQty
      ).run();
      
      planId = planResult.meta.last_row_id as number;
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        `).bind(
          planId, i + 1, item.product_name, item.product_code, item.quantity,
          currentStock, frozenStock, requiredQty > 0 ? requiredQty : 0
        ).run();
      }
    }
    ...
    return c.json({
      ...
      data: {
        ...
        is_new: !existingPlan,
        items_count: items.length,
        message: existingPlan ? '기존 스케줄에 추가되었습니다.' : '새 생산 스케줄이 생성되었습니다.'
      }
    });
    
  } catch (error: any) {
    console.error('Production schedule creation error:', error);
    return c.json({ success: false, error: error.message || '스케줄 생성 중 오류 발생' }, 500);
  }
});

// 생산 스케줄에서 원료 소요량 계산 (BOM 기반)
productionPlanRoutes.get('/:id/material-requirements', async (c) => {
  const planId = c.req.param('id');
  
  try {
    // 스케줄 품목 조회
    const items = await c.env.DB.prepare(`
      SELECT product_code, product_name, order_total, required_qty 
      FROM production_plan_items 
      WHERE plan_id = ? AND required_qty > 0
    `).bind(planId).all();
    
    const materialMap = new Map<string, { item_code: string; item_name: string; total_qty: number; unit: string }>();
    
    for (const item of items.results as any[]) {
      // BOM 조회
      const bom = await c.env.DB.prepare(`
        SELECT b.item_code, b.quantity, b.unit, m.item_name
        FROM bom b
        LEFT JOIN master m ON b.item_code = m.item_code
        WHERE b.product_code = ?
      `).bind(item.product_code).all();
      
      for (const bomItem of bom.results as any[]) {
        const requiredQty = bomItem.quantity * item.required_qty;
        
        if (materialMap.has(bomItem.item_code)) {
          const existing = materialMap.get(bomItem.item_code)!;
          existing.total_qty += requiredQty;
        } else {
          materialMap.set(bomItem.item_code, {
    ...
    // 재고와 비교
    const materials = [];
    for (const [code, mat] of materialMap) {
      const stock = await c.env.DB.prepare(`
        SELECT current_stock FROM master WHERE item_code = ?
      `).bind(code).first<{ current_stock: number }>();
      
      const currentStock = stock?.current_stock || 0;
      const shortage = mat.total_qty - currentStock;
    ...
    // 부족분이 있는 것 먼저 정렬
    materials.sort((a, b) => (b.is_short ? 1 : 0) - (a.is_short ? 1 : 0));
