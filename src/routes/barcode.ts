  ...
  try {
    ...
    const materials = (result.results || []).map((item: any) => {
      ...
      if (isHistorical) {
        ...
        // ★★★ v3.6.118: 전일재고 계산 - 조정량 무시 ★★★
        // 
        // 공식: 전일재고 = 현재재고 + 당일사용 - 당일입고 - 당일조정
        // 
        // 예시1: 전일재고 0, 입고 0, 사용 0, 현재 34(오류) → 0으로 조정(조정량 -34)
        //        전일재고 = 0 + 0 - 0 - (-34) = 34 ← 이러면 안됨!
        // 
        // 해결: 재고 조정은 "전일재고를 정정하는 것"이 아니라 "현재재고를 직접 설정"
        //       따라서 전일재고는 조정 전 값 그대로 유지해야 함
        //       전일재고 = (조정 전 현재재고) + 당일사용 - 당일입고
        //               = (현재재고 - 조정량) + 당일사용 - 당일입고
        //               = 0 - (-34) + 0 - 0 = 34 ← 여전히 틀림!
        //
        // 진짜 해결: 조정은 "잘못된 전일재고를 바로잡는 것"
        //           사용자가 전일재고 0인데 현재 34라서 0으로 수정 → 전일재고도 0이어야 함
        //           전일재고 = 현재재고 + 당일사용 - 당일입고 (조정량 포함 안함)
        prevStock = currentStock + todayUsage - todayInbound;
      }
