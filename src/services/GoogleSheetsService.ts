...
export class GoogleSheetsService {
  ...
  async rebuildLotMatchingForDates(dates: string[]): Promise<{ 
    ...
    // ★★★ v3.6.104: 원료입고 시트 구조 매핑 수정 - 잔량(I열) 사용 ★★★
    ...
    for (const row of inboundData) {
      ...
      const expiry = row[7]?.toString() || '';        // H열: 소비기한
      const qty = parseFloat(row[4]?.toString() || '0') || 0;  // E열: 입고량
      const remaining = parseFloat(row[8]?.toString() || '0') || 0;  // I열: 잔량 (★★★ v3.6.104 수정)
      
      if (!materialCode || qty <= 0) continue;
      // ★★★ v3.6.104: 잔량이 0 이하인 LOT는 건너뛰기 ★★★
      if (remaining <= 0) continue;
      
      if (!inboundByMaterial[materialCode]) {
        inboundByMaterial[materialCode] = [];
      }
      // ★★★ v3.6.104: remaining은 실제 잔량(I열) 사용 ★★★
      inboundByMaterial[materialCode].push({ lot, expiry, qty, remaining });
    }
  ...
  async addDailyStockDate(targetDate: string): Promise<{ date: string; prev_date: string; new_rows: number; total_rows: number; write_success?: boolean; write_error?: string }> {
    ...
    console.log(`[addDailyStockDate v3.6.43] 로트매칭에서 ${Object.keys(usageMap).length}개 원료 사용량 계산`);
    
    // 6. 새 날짜 행 생성 - 품목코드 순 정렬
    const sortedCodes = Object.keys(prevStockMap).sort();
    const newRows: any[][] = [];
    
    // ★★★ v3.6.43: 현재 일별수불부 시트의 마지막 행 번호 확인 ★★★
    ...
    for (let i = 0; i < sortedCodes.length; i++) {
      ...
      // ★★★ v3.6.43: 전일재고는 값으로 직접 입력 (수식 순환참조 방지) ★★★
